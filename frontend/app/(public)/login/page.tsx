...
// Animated agent workflow panel — pure CSS/SVG, no external image needed
function AgentPanel() {
  const agents = [
    ...
    <div className="relative w-full h-full flex flex-col justify-center px-10 py-12 overflow-hidden select-none"
      ...
      <div className="relative z-10 rounded-[16px] border border-white/[0.06] overflow-hidden"
        ...
        <div className="px-4 py-3 flex flex-col gap-1.5">
          {logLines.map((l, i) => (
            <div key={i} className="flex items-start gap-2 font-mono text-[11px]" style={{ animationDelay: `${i * 0.15}s` }}>
      ...
      <div className="relative z-10 flex flex-wrap gap-2 mt-5">
        {[
          { label: 'Claude', status: 'Completed', color: '#a78bfa' },
          { label: 'n8n', status: 'Running', color: '#fb923c', pulse: true },
          { label: 'OpenClaw', status: 'Completed', color: '#34d399' },
          { label: 'OrchestrAI', status: 'Routing', color: '#6366f1', pulse: true },
        ].map((a) => (
          <div key={a.label}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full border"
            style={{ background: `${a.color}10`, borderColor: `${a.color}30` }}>
            <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${a.pulse ? 'animate-pulse' : ''}`} style={{ background: a.color }} />
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/workspaces', { headers: { Authorization: `Bearer ${session?.access_token}` } })
  ...
  return (
    <div className="min-h-screen flex bg-bg overflow-hidden font-sans">
      ...
      <div className="flex flex-col justify-center w-full max-w-[480px] flex-shrink-0 px-12 py-16 relative z-10">
        {/* Very faint glow behind the form */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 relative" noValidate>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              <input id="password" type={show ? 'text' : 'password'} autoComplete="current-password" required value={password}
      ...
      {/* RIGHT — Animated agent panel, takes the rest of the page */}
